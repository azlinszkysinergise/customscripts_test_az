//VERSION=3 - created with AI, not tested on real imagery yet

var tonemapMethod = 4; // 0 - Simple Reinhard
...
var atm = (a, ii) => (typeof cManual !== 'undefined') ? 
    ((cManual[ii] instanceof Array) ? atm2p(a, cManual[ii][0], cManual[ii][1]) : atm1p(a, cManual[ii])) 
...
function Uncharted2FilmicCurve(x) {
    // Uncharted2 tone mapping (See https://filmicgames.com)
    const A = 0.15;
    const B = 0.50;
    const C = 0.10;
    const D = 0.20;
...
function tonemap(col) {
    switch (tonemapMethod) {
        ...
        case 1: return LumaBasedReinhardTonemap(col);
...
// SETUP: Request PlanetScope bands & DataMask
function setup() {
    return {
        input: ["red", "green", "blue", "dataMask"],
        output: { bands: 4 } // 4 Bands for RGB + Alpha
    };
}

// EVALUATE PIXEL: Pixel-by-pixel rendering logic
function evaluatePixel(sample) {
    // Mask out no-data pixels perfectly transparent
    if (sample.dataMask !== 1) {
        return [0, 0, 0, 0];
    }

    // Convert PlanetScope DN values (0-10000) to reflectance (0.0-1.0)
    // Order strictly as [Red, Green, Blue]
    var col = [
        sample.red / 10000, 
        sample.green / 10000, 
        sample.blue / 10000
    ...
    // Apply solar irradiance correction
    if (adjForSunColor) col = [col[0], 0.939 * col[1], 0.779 * col[2]];
    col = col.map(a => a * gain);

    if (debug) {
        var minC = Math.min.apply(null, col);
        if (minC < 0.) col = [0., 0., 1.]; // Highlight blue if below 0
    }

    // Apply saturation and tonemapping
    col = Saturate(col);
    col = tonemap(col);

    if (debug) {
        var maxC = Math.max.apply(null, col);
        if (maxC > 1.) col = [1., 0., 0.]; // Highlight red if blown out
    }

    col = col.map(sRGBCurve);

    // Return final mapped RGB values and the data mask as Alpha channel
    return [col[0], col[1], col[2], sample.dataMask];
}
